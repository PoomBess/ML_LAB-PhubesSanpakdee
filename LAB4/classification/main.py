...
def main():
    ...
    print(
        f"split data   : train {len(y_train)} / validation {len(y_val)} / test"
        f" {len(y_test)}"
    )

    title("STEP 2 : search k for finding the best value")

    # กรองค่า k ไม่ให้เกินจำนวนข้อมูลใน X_train
    max_k = len(X_train)
    k_candidates = [1, 2, 3, 5, 10, 11, 15, 21, 30]
    k_values = [k for k in k_candidates if k <= max_k]
    ...
    print(
        "KNN: matching predictions      :"
        f" {np.mean(sk_pred == y_pred) * 100:.1f}%"
    )
    ...
    print(
        f"Baseline (predict '{class_names[majority]}' every time) :"
        f" {baseline:.4f}"
    )
    ...
    if accuracy <= baseline:
        ...
        print("         detect 'not signal in data' is important skill")
